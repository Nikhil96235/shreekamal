...
- **Splash popup animation strong ki:** `skZoom` ko single clean bounce banaya — `1.4s cubic-bezier(.2,.8,.3,1)`, keyframes `scale 0.6→(70%)1.07→1.0` + opacity fade. `.sk-intro` base background bhi gradient kiya (taaki popup ke waqt scenic backdrop dikhe, white flash na ho). User ne double-bounce reject kiya tha → single overshoot rakha.
